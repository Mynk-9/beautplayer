...
const QueueManager = {
    ...
    removeTrack: trackId => {
        ...
        for (; i < BeautPlayerQueue.queue.length; ++i)
            if (trackId === BeautPlayerQueue.queue[i].trackId) break;
        if (i >= BeautPlayerQueue.queue.length) return;
        BeautPlayerQueue.queue.splice(i, 1);
        PlayerManager.getInstance().forcePrefetch();
    },
    ...
    getNextTrack: (currentTrack, shuffle) => {
        ...
        for (i = 0; i < BeautPlayerQueue.queue.length; ++i)
            if (BeautPlayerQueue.queue[i].trackId === currentTrack) break;
        nextIndex = i + 1;

        if (nextIndex >= BeautPlayerQueue.queue.length)
            // check
            return null;
        ...
        if (shuffle) {
            ...
            BeautPlayerQueue.queue[nextIndex] =
                BeautPlayerQueue.queue[randomIndex];
    ...
    getPrevTrack: currentTrack => {
        ...
        for (i = 0; i < BeautPlayerQueue.queue.length; ++i)
            if (BeautPlayerQueue.queue[i].trackId === currentTrack) break;
        --i; // prev track
        if (i < 0 || i >= BeautPlayerQueue.queue.length)
            // check
            return null;
    ...
    checkQueue: trackId => {
        for (let i = 0; i < BeautPlayerQueue.queue.length; ++i)
            if (trackId === BeautPlayerQueue.queue[i].trackId) return true;
        return false;
    },
};
