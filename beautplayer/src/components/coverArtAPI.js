...
export function playlistArt(playlistName) {
    return API + '/coverart/playlists/' + playlistName.replace(/[^a-z0-9]/gi, '_').toLowerCase() + '.jpg';
}
export function playlistArtCompressed(playlistName) {
    return API + '/coverart/playlists/compressed/' + playlistName.replace(/[^a-z0-9]/gi, '_').toLowerCase() + '.jpg';
}